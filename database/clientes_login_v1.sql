-- Mica Store Clientes Login v1
-- Ejecutar dentro de mica_store. No borra datos.
...
-- Si tu tabla cotizaciones no tiene cliente_web_id, se agrega de forma compatible
SET @db = DATABASE();

SET @existe := (
    SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA=@db AND TABLE_NAME='cotizaciones' AND COLUMN_NAME='cliente_web_id'
);
